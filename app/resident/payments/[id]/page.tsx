...
export default async function ResidentPaymentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { request, payment } = await getResidentDemoPayment(id);
  if (!request) redirect("/resident/my-requests?error=Payment%20request%20not%20found.");
  return <div className="mx-auto max-w-2xl space-y-5">
    <div><h1 className="text-3xl font-bold">Demo Payment</h1><p className="text-base-content/70">No actual funds are transferred in this thesis demonstration.</p></div>
    <div className="alert alert-warning"><span>DEMO PAYMENT - NO ACTUAL FUNDS TRANSFERRED</span></div>
    ...
    {!payment && request.status === "accepted" && request.payment_status === "unpaid" ? <form action={startMockPaymentAction}><input type="hidden" name="request_id" value={request.id}/><button className="btn btn-primary">Start Demo Payment</button></form> : null}
    {payment?.status === "pending" ? <div className="flex flex-wrap gap-2"><form action={resolveMockPaymentAction}><input type="hidden" name="payment_id" value={payment.id}/><input type="hidden" name="request_id" value={request.id}/><input type="hidden" name="outcome" value="paid"/><button className="btn btn-success">Simulate Successful Payment</button></form><form action={resolveMockPaymentAction}><input type="hidden" name="payment_id" value={payment.id}/><input type="hidden" name="request_id" value={request.id}/><input type="hidden" name="outcome" value="failed"/><button className="btn btn-outline">Simulate Failed Payment</button></form><form action={resolveMockPaymentAction}><input type="hidden" name="payment_id" value={payment.id}/><input type="hidden" name="request_id" value={request.id}/><input type="hidden" name="outcome" value="cancelled"/><button className="btn btn-ghost">Cancel Demo Payment</button></form></div> : null}
    {payment ? <p className="text-sm text-base-content/70">Reference: {payment.provider_transaction_id} ({payment.status})</p> : null}
